...
export default function BlueprintComponent({ onPerimeterChange }: { onPerimeterChange: (p: number) => void }) {
  ...
  const addPointToDrawing = (worldPos: Point) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-100 relative overflow-hidden overscroll-none" ref={containerRef}>
      ...
      {/* Mobile Sniper Button (Calibration) */}
      {isCalibrating && !showCalibrationPrompt ? (
        <div className="md:hidden absolute bottom-8 left-1/2 -translate-x-1/2 pointer-events-none z-30 flex justify-center w-full px-6">
          <button 
